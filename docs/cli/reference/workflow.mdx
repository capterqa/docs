...
steps:
  - name: create product
    ...
    assertions:
      - !assert status to_equal 201
      - !assert body.name to_equal ${{ env.product_name }}
...
steps:
  ...
  - name: fetch logged in user
    ...
    assertions:
      - !assert body.email to_equal ${{ env.email }}
```
...
assertions:
  - !assert status to_equal 200
  - !assert duration to_be_below 4000
  - !assert body.email to_equal filip@capter.io
```
