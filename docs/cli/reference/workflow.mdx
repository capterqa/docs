...
```yml title="./capter/product-crud.yml"
name: product crud
env:
...
steps:
  - name: create product
    ...
    assertions:
      - !assert status equal 201
      - !assert body.name equal ${{ env.product_name }}
...
steps:
  ...
  - name: fetch logged in user
    ...
    assertions:
      - !assert body.email equal ${{ env.email }}
```
...
assertions:
  ...
  - !assert duration isBelow 4000
  - !assert body.email is filip@capter.io
```
