...
steps:
  - name: fetch a products
    ...
    assertions:
      - !assert body.name to_equal M1 MacBook Air
```
...
```yml
!assert data.age to_be_above 30
```
...
```yml
!assert data.products to_be_array
```
...
```yml
!assert data.age to_be_at_least 30
```
...
```yml
!assert data.age to_be_at_most 30
```
...
```yml
!assert data.age to_be_below 30
```
...
```yml
!assert data.products to_be_boolean
```
...
```yml
!assert data.products to_be_empty
```
...
```yml
!assert data.is_admin to_be_false
```
...
```yml
!assert data.price to_be_number
```
...
```yml
!assert data.email to_be_string
```
...
```yml
!assert data.is_admin to_be_true
```
...
```yml
!assert data.errors to_be_undefined
```
...
```yml
!assert data.products to_contain M1 MacBook Air
!assert data.names to_contain Steve
!assert data.name to_contain Jobs
```
...
```yml
!assert data.name to_equal Steve
```
...
```yml
!assert data.products to_have_length 5
```
...
```yml
!assert data.products to_match M1 MacBook Air
```
